        ...
        if(err.code===3){
          setTimeout(function(){
            if(watchId.current!==null){
              ...
              watchId.current=navigator.geolocation.watchPosition(
                function(p){setPos({lat:p.coords.latitude,lng:p.coords.longitude,acc:p.coords.accuracy});saveGPSPoint(user.id,p.coords.latitude,p.coords.longitude,user);sl('gps_last_'+user.id,{lat:p.coords.latitude,lng:p.coords.longitude,ts:Date.now(),nombre:user.nombre||user.username});setGpsErr('');},
                function(){},
