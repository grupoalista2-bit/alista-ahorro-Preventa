...
function boolPesable(v){var x=normTxt(v);return v===true||x==='si'||x==='sí'||x==='s'||x==='true'||x==='1'||x==='x'||x==='kg'||x==='pesable'||x==='balanza';}
...
var ART_ALIAS_PESABLE=['PESABLE','PESABLE?','ES_PESABLE','ES PESABLE','BALANZA','POR KG','POR_KG','KG','PESO','VENTA POR KG','SE VENDE POR KG'];
...
function turnoToDb(t){return {
  ...
  resumen:t.resumen||{},observaciones:t.observaciones||'',created_at:t.created_at||null,updated_at:t.updated_at||null
};}
...
function NuevoPedido(props){
  ...
  function addArt(a){
    ...
    if(idx>=0){var u=items.slice();var inc=itemEsPesable(u[idx])?0.1:1;u[idx]=Object.assign({},u[idx],{cant:normalizarCantidad((parseFloat(u[idx].cant)||0)+inc,itemEsPesable(u[idx]),itemEsPesable(u[idx])?0.001:1)});setItems(u);}
      ...
      artId:a.id,cod:a.cod,codArt:a.codArt,desc:a.desc,cant:1,pesable:!!a.pesable,
          ...
          E('table',null,
            ...
            E('tbody',null,items.map(function(it,i){
              ...
              return E('tr',{key:i},
                ...
                E('td',null,E('input',{className:'fi sm qty',type:'number',min:itemEsPesable(it)?0.001:1,step:qtyStep(it),value:it.cant,onChange:function(e){updItem(i,'cant',e.target.value);}}),E('div',{style:{fontSize:10,color:'var(--txt2)'}},qtyLabel(it))),
  ...
  function reemplazar(idx,art){
    setPrepItems(function(prev){
      ...
      var rep={artId:art.id,cod:art.cod,codArt:art.codArt,desc:art.desc,
        cant:u[idx].cant,cantFinal:u[idx].cant,pesable:!!art.pesable,pu:artPrecioPublicoUnit(art),costo:artCostoUnitFromArt(art),precioPublico:artPrecioPublicoUnit(art),
  ...
  function agregarArt(art){
    setPrepItems(function(prev){
      return prev.concat([{artId:art.id,cod:art.cod,codArt:art.codArt,
        desc:art.desc,cant:1,cantFinal:1,pesable:!!art.pesable,pu:artPrecioPublicoUnit(art),costo:artCostoUnitFromArt(art),precioPublico:artPrecioPublicoUnit(art),
      ...
      E('div',{className:'tw',style:{marginBottom:12}},E('table',null,
        ...
        E('tbody',null,prepItems.map(function(it,i){
          ...
          return E('tr',{key:i,style:{background:bg}},
            ...
            E('td',null,E('input',{type:'number',min:0,max:it.tipo==='agregado'?999:it.cant,step:qtyStep(it),value:it.cantFinal,
              ...
              style:{width:64,padding:'5px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontFamily:'inherit',
                background:it.cantFinal===0?'#fee2e2':changed?'#fef9c3':''}}),E('div',{style:{fontSize:10,color:'var(--txt2)'}},qtyLabel(it))),
            E('td',null,
              isAdminPrep
