...
function OfertasPreventistas(props){
  ...
  var _artQ=useState(''),artQ=_artQ[0],setArtQ=_artQ[1];
  var _artRes=useState([]),artRes=_artRes[0],setArtRes=_artRes[1];
  var _flyer=useState(false),flyer=_flyer[0],setFlyer=_flyer[1];

  function flash(t,m){setMsg({t:t,m:m});setTimeout(function(){setMsg(null);},4500);}
  function emptyForm(){
    var d=(new Date()).toISOString().slice(0,10);
    return {id:uid(),articuloId:'',cod:'',codArt:'',descripcion:'',precioRegular:0,precioOferta:0,costo:0,fechaDesde:d,fechaHasta:'',stockInfo:'',observacion:'',fotoUrl:'',mostrarFlyer:true,mostrarPrecioAnterior:true,mostrarVigencia:true,ordenFlyer:0,activo:true,createdBy:user.id||'',coincidenciaDudosa:false,estadoRevision:'ok',revisionMotivo:''};
  }
  function reload(){
    setLoading(true);
    Promise.all([
      dbGet('ofertas_preventistas').catch(function(e){console.error(e);return [];}),
      dbGetSelect('articulos','id,cod,cod_art,descripcion,precio,costo,estado,pesable',{limit:300,order:'descripcion.asc'}).catch(function(){return [];})
    ]).then(function(res){
      setOfertas((res[0]||[]).map(dbToOferta).sort(function(a,b){return (parseInt(a.ordenFlyer)||0)-(parseInt(b.ordenFlyer)||0)||String(a.descripcion||'').localeCompare(String(b.descripcion||''),'es');}));
      setArts((res[1]||[]).map(dbToArt));
      setLoading(false);
    }).catch(function(e){setLoading(false);flash('err','No pude cargar ofertas: '+(e.message||e));});
  }
  useEffect(function(){reload();},[]);
  useEffect(function(){
    var term=String(artQ||'').trim();
    if(term.length<2){setArtRes([]);return;}
    var cancel=false;
    dbSearchArticulos(term).then(function(rows){if(!cancel)setArtRes((rows||[]).map(dbToArt));}).catch(function(){if(!cancel)setArtRes([]);});
    return function(){cancel=true;};
  },[artQ]);
  function F(k,v){setForm(function(f){return Object.assign({},f,{[k]:v});});}
  function selectArt(a){
    setForm(function(f){return Object.assign({},f,{articuloId:a.id||'',cod:a.cod||'',codArt:a.codArt||'',descripcion:a.desc||'',precioRegular:artPrecioPublicoUnit(a)||0,costo:parseFloat(a.costo)||0});});
    setArtQ(a.desc||'');setArtRes([]);
  }
  function guardar(){
    if(!isAdminOrCo){flash('err','Solo administrador puede modificar ofertas.');return;}
    if(!form||!String(form.descripcion||'').trim()){flash('err','Elegí o escribí un artículo.');return;}
    if(!(parseFloat(form.precioOferta)>0)){flash('err','Ingresá un precio de oferta válido.');return;}
    var row=ofertaToDb(Object.assign({},form,{precioRegular:parseFloat(form.precioRegular)||0,precioOferta:parseFloat(form.precioOferta)||0,costo:parseFloat(form.costo)||0,ordenFlyer:parseInt(form.ordenFlyer)||0,createdBy:form.createdBy||user.id||''}));
    dbUpsert('ofertas_preventistas',row).then(function(){setModal(null);reload();flash('ok','Oferta guardada.');}).catch(function(e){flash('err','Error al guardar oferta: '+(e.message||e));});
  }
  function toggle(o){
    if(!isAdminOrCo)return;
    dbUpdate('ofertas_preventistas',o.id,{activo:o.activo===false?true:false}).then(reload).catch(function(e){flash('err','No pude actualizar: '+(e.message||e));});
  }
  function eliminar(o){
    if(!isAdminOrCo)return;
    if(!confirm('¿Eliminar esta oferta relámpago?'))return;
    dbDelete('ofertas_preventistas',o.id).then(function(){reload();flash('ok','Oferta eliminada.');}).catch(function(e){flash('err','No pude eliminar: '+(e.message||e));});
  }
  function abrirNueva(){setForm(emptyForm());setArtQ('');setArtRes([]);setModal('edit');}
  function abrirEdit(o){setForm(Object.assign({},emptyForm(),o));setArtQ(o.descripcion||'');setArtRes([]);setModal('edit');}
  function getList(){
    var t=normTxt(q);
    return ofertas.filter(function(o){
      if(!t)return true;
      return normTxt([o.descripcion,o.cod,o.codArt,o.stockInfo,o.observacion,o.estadoRevision].join(' ')).indexOf(t)>=0;
    });
  }
  var hoy=(new Date()).toISOString().slice(0,10);
  var list=getList();
  var activas=list.filter(function(o){return ofertaVigente(o,hoy);});
  var flyerList=ofertas.filter(function(o){return ofertaVigente(o,hoy)&&o.mostrarFlyer!==false;}).sort(function(a,b){return (parseInt(a.ordenFlyer)||0)-(parseInt(b.ordenFlyer)||0);});

  return E('div',null,
    E('div',{className:'card'},
      E('div',{className:'brow',style:{justifyContent:'space-between',alignItems:'center'}},
        E('div',null,E('h2',null,'🔥 Ofertas Relámpago'),E('p',{className:'muted'},isAdminOrCo?'Administrá ofertas, precios, flyer y vigencia.':'Ofertas vigentes para vender con precio especial.')),
        E('div',{className:'brow'},
          E('button',{className:'btn',onClick:reload},'Actualizar'),
          isAdminOrCo&&E('button',{className:'btn pri',onClick:abrirNueva},'+ Nueva oferta'),
          E('button',{className:'btn',onClick:function(){setFlyer(true);}},'🖨️ Flyer')
        )
      ),
      msg&&E('div',{className:'alert '+(msg.t||'ok')},msg.m),
      E('div',{className:'grid2'},
        E('div',{className:'stat'},E('div',{className:'stat-label'},'Ofertas vigentes'),E('div',{className:'stat-val'},activas.length)),
        E('div',{className:'stat'},E('div',{className:'stat-label'},'En flyer'),E('div',{className:'stat-val'},flyerList.length))
      ),
      E('input',{className:'inp',placeholder:'Buscar oferta por producto, código o estado…',value:q,onChange:function(e){setQ(e.target.value);},style:{marginTop:10}})
    ),
    loading&&E('div',{className:'card'},'Cargando ofertas…'),
    !loading&&E('div',{className:'grid cards'},list.map(function(o){
      var vigente=ofertaVigente(o,hoy);
      return E('div',{className:'card',key:o.id,style:{borderColor:vigente?'var(--border)':'#f3c6c6'}},
        E('div',{style:{display:'flex',gap:10,alignItems:'flex-start'}},
          o.fotoUrl?E('img',{src:o.fotoUrl,style:{width:74,height:74,objectFit:'cover',borderRadius:14,border:'1px solid var(--border)'}}):E('div',{style:{width:74,height:74,borderRadius:14,border:'1px dashed var(--border)',display:'grid',placeItems:'center',color:'var(--txt2)'}},'Foto'),
          E('div',{style:{flex:1}},
            E('div',{style:{fontWeight:800}},o.descripcion||'Sin descripción'),
            E('div',{className:'muted',style:{fontSize:12}},'Cod: '+(o.cod||'—')+(o.codArt?' · Art: '+o.codArt:'')),
            o.coincidenciaDudosa&&E('div',{className:'alert warn',style:{margin:'6px 0',padding:'6px 8px'}},'⚠ Revisar coincidencia'),
            E('div',{style:{display:'flex',gap:8,alignItems:'baseline',marginTop:6}},
              o.precioRegular?E('del',{className:'muted'},'$'+$(o.precioRegular)):null,
              E('strong',{style:{fontSize:22,color:'var(--red)'}},'$'+$(o.precioOferta))
            ),
            E('div',{className:'muted',style:{fontSize:12}},'Vigencia: '+(o.fechaDesde||'—')+' a '+(o.fechaHasta||'sin fecha')+(o.stockInfo?' · '+o.stockInfo:''))
          )
        ),
        isAdminOrCo&&E('div',{className:'brow',style:{marginTop:10}},
          E('button',{className:'btn sm',onClick:function(){abrirEdit(o);}},'Editar'),
          E('button',{className:'btn sm',onClick:function(){toggle(o);}},o.activo===false?'Activar':'Pausar'),
          E('button',{className:'btn sm danger',onClick:function(){eliminar(o);}},'Eliminar')
        )
      );
    })),
    !loading&&!list.length&&E('div',{className:'card muted'},'No hay ofertas para mostrar.'),
    modal&&form&&E(Modal,{title:form.id?'Oferta Relámpago':'Nueva Oferta',onClose:function(){setModal(null);},wide:true},
      E('div',{className:'grid2'},
        E('div',null,E('label',null,'Buscar artículo'),E('input',{className:'inp',value:artQ,onChange:function(e){setArtQ(e.target.value);},placeholder:'Escribí 2 letras…'}),
          artRes.length>0&&E('div',{className:'card',style:{padding:6,maxHeight:180,overflow:'auto',marginTop:6}},artRes.slice(0,20).map(function(a){return E('button',{key:a.id,className:'rowbtn',onClick:function(){selectArt(a);}},E('strong',null,a.desc),E('span',null,' $'+$(artPrecioPublicoUnit(a))+' · '+(a.cod||'')));}))
        ),
        E('div',null,E('label',null,'Descripción'),E('input',{className:'inp',value:form.descripcion||'',onChange:function(e){F('descripcion',e.target.value);}})),
        E('div',null,E('label',null,'Código'),E('input',{className:'inp',value:form.cod||'',onChange:function(e){F('cod',e.target.value);}})),
        E('div',null,E('label',null,'Código proveedor/artículo'),E('input',{className:'inp',value:form.codArt||'',onChange:function(e){F('codArt',e.target.value);}})),
        E('div',null,E('label',null,'Precio regular'),E('input',{className:'inp',type:'number',step:'0.01',value:form.precioRegular||'',onChange:function(e){F('precioRegular',e.target.value);}})),
        E('div',null,E('label',null,'Precio oferta'),E('input',{className:'inp',type:'number',step:'0.01',value:form.precioOferta||'',onChange:function(e){F('precioOferta',e.target.value);}})),
        E('div',null,E('label',null,'Desde'),E('input',{className:'inp',type:'date',value:form.fechaDesde||'',onChange:function(e){F('fechaDesde',e.target.value);}})),
        E('div',null,E('label',null,'Hasta'),E('input',{className:'inp',type:'date',value:form.fechaHasta||'',onChange:function(e){F('fechaHasta',e.target.value);}})),
        E('div',null,E('label',null,'Stock / condición'),E('input',{className:'inp',value:form.stockInfo||'',onChange:function(e){F('stockInfo',e.target.value);},placeholder:'Hasta agotar stock / x bulto / etc.'})),
        E('div',null,E('label',null,'Orden flyer'),E('input',{className:'inp',type:'number',value:form.ordenFlyer||0,onChange:function(e){F('ordenFlyer',e.target.value);}}))
      ),
      E('label',{style:{display:'block',marginTop:10}},E('input',{type:'checkbox',checked:form.activo!==false,onChange:function(e){F('activo',e.target.checked);}}),' Oferta activa'),
      E('label',{style:{display:'block',marginTop:6}},E('input',{type:'checkbox',checked:form.mostrarFlyer!==false,onChange:function(e){F('mostrarFlyer',e.target.checked);}}),' Mostrar en flyer'),
      E('div',{style:{marginTop:10}},E('label',null,'Observación'),E('textarea',{className:'inp',value:form.observacion||'',onChange:function(e){F('observacion',e.target.value);},rows:2})),
      E('div',{className:'brow',style:{marginTop:12}},E('button',{className:'btn pri',onClick:guardar,style:{flex:1}},'Guardar oferta'),E('button',{className:'btn',onClick:function(){setModal(null);},style:{flex:1}},'Cancelar'))
    ),
    flyer&&E(Modal,{title:'Flyer Ofertas Relámpago',onClose:function(){setFlyer(false);},wide:true},
      E('div',{className:'flyer-print'},
        E('h1',{style:{textAlign:'center',color:'var(--red)',margin:'0 0 8px'}},'OFERTA RELÁMPAGO'),
        E('p',{style:{textAlign:'center',margin:'0 0 12px'}},BIZ.nombre+' · WhatsApp '+BIZ.wa),
        E('div',{className:'grid cards'},flyerList.slice(0,8).map(function(o){return E('div',{className:'card',key:o.id},
          o.fotoUrl&&E('img',{src:o.fotoUrl,style:{width:'100%',height:120,objectFit:'cover',borderRadius:12}}),
          E('strong',null,o.descripcion),
          E('div',null,o.precioRegular?E('del',{className:'muted'},'$'+$(o.precioRegular)):null),
          E('div',{style:{fontSize:26,fontWeight:900,color:'var(--red)'}},'$'+$(o.precioOferta)),
          o.fechaHasta&&E('small',{className:'muted'},'Hasta '+o.fechaHasta)
        );}))
      ),
      E('div',{className:'brow',style:{marginTop:12}},E('button',{className:'btn pri',onClick:function(){window.print();}},'Imprimir / Guardar PDF'),E('button',{className:'btn',onClick:function(){setFlyer(false);}},'Cerrar'))
    )
  );
}
